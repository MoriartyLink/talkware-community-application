-- Talkware complete clean-install schema.
-- Existing deployments should apply the ordered files in supabase/migrations instead.

-- Talkware Community Landing baseline database schema
...
create index if not exists point_ledger_member_earned_idx
  on public.point_ledger(member_id, earned_at desc);
create index if not exists point_ledger_event_idx
...
drop trigger if exists award_event_attendance_points on public.event_attendance;
...
declare
  ...
  policy_name text;
begin
  foreach table_name in array array['co_creators','volunteers','founding_team','contributor_tags','contributors','event_media','event_sections']
  loop
...
create policy "Members read own point ledger" on public.point_ledger for select to authenticated
  using ((select auth.uid()) = member_id or private.is_staff());

grant select on public.events, public.co_creators, public.volunteers,
...
grant select, insert, update, delete on public.events, public.co_creators,
...
create policy "Members upload own avatars" on storage.objects for insert to authenticated
  with check (bucket_id = 'assets' and (storage.foldername(name))[1] = 'member-avatars' and (storage.foldername(name))[2] = (select auth.uid())::text and private.is_approved_member());
drop policy if exists "Members update own avatars" on storage.objects;
create policy "Members update own avatars" on storage.objects for update to authenticated
  using (bucket_id = 'assets' and (storage.foldername(name))[1] = 'member-avatars' and (storage.foldername(name))[2] = (select auth.uid())::text)
  with check (bucket_id = 'assets' and (storage.foldername(name))[1] = 'member-avatars' and (storage.foldername(name))[2] = (select auth.uid())::text and private.is_approved_member());
drop policy if exists "Members delete own avatars" on storage.objects;
create policy "Members delete own avatars" on storage.objects for delete to authenticated
  using (bucket_id = 'assets' and (storage.foldername(name))[1] = 'member-avatars' and (storage.foldername(name))[2] = (select auth.uid())::text);
